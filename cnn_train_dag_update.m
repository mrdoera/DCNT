...
opts.solver = @solver.adam_CREST; % Empty array - optimised SGD solver
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    sample1{1}=input{1}{1}{1};
    sample1{2}=input{1}{2}{1};
    
    sample2{1}=input{1}{1}{2};
    sample2{2}=input{1}{2}{2};
    
    sample3{1}=input{1}{1}{3};
    sample3{2}=input{1}{2}{3};
...
%     labels=zeros(hs,ws,1,num_update,'single');
    input1=zeros(hs,ws,cs,num_update,'single');           
    input2=zeros(hs,ws,cs,num_update,'single');
    input3=zeros(hs,ws,cs,num_update,'single');
    input4=zeros(hs,ws,cs,num_update,'single');
    labels=zeros(hs,ws,1,num_update,'single');
    for i=1:num_update
        input1(:,:,:,i)=sample1{i};
        input2(:,:,:,i)=sample2{i};
